...
if __name__ == "__main__":
    ...
    # Set file paths
    original_file = os.path.join(output_dir, "entity_relationships.gexf")
    output_path = os.path.join(output_dir, "entity_clusters.png")
    
    # Check if the file exists
    if not os.path.exists(original_file):
        print(f"Entity relationship file not found: {original_file}")
        print("Please run process_documents.py first to generate the file.")
        sys.exit(1)
    ...
    if nodes:
        ...
        create_clustered_graph(nodes, output_path)
    else:
